...
type SparringMessage = { role: "user" | "assistant"; assistant: SparringAssistant; content: string; matrix?: ScheduleMatrixRow[]; entityReferences?: EntityReference[]; card?: WorkspaceCard };
    ...
    <div className={styles.matrixTableScroll}>
      <table className={styles.matrixTable}>
        ...
        <tbody>{filtered.map((row, index) => {
          ...
          const taskAuthorized = row.type === "task" && row.taskId ? accessibleTaskIds.has(row.taskId) : false;
...
export function WorkflowShell({
  ...
  // V12-Nachschärfung 4: task-scharfe Berechtigung für Zeilen der Terminmatrix, siehe
  // loadFullScheduleMatrix() - befüllt über dieselbe tasks_access_select-RLS wie überall sonst.
  const [accessibleTaskIds, setAccessibleTaskIds] = useState<Set<string>>(new Set());
  ...
  async function loadFullScheduleMatrix(assistant: SparringAssistant) {
    try {
      ...
      setAccessibleTaskIds(new Set((accessibleTaskRows || []).map((row: any) => String(row.id))));
