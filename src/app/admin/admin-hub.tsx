...
type AdminTab = "users" | "companies" | "projects" | "measures" | "permissions" | "transfer";
type MeasureSort = "process" | "due" | "role" | "number";
...
const isTab=(v:string|null):v is AdminTab=>v==="users"||v==="companies"||v==="projects"||v==="measures"||v==="permissions"||v==="transfer";
...
export default function AdminHub({
  ...
  const [measureSort,setMeasureSort]=useState<MeasureSort>("process");
  const [openMeasurePhases,setOpenMeasurePhases]=useState<Set<number>>(()=>new Set());
  const [transferMessage,setTransferMessage]=useState("");
  const workbookFrameRef=useRef<HTMLIFrameElement|null>(null);
  ...
  useEffect(()=>{
    const handler=(event:MessageEvent)=>{
      if(event.origin!==window.location.origin||event.data?.type!=="lumina-admin-workbook-result")return;
      if(event.data.ok)setTransferMessage(event.data.action==="import"?"Workbook-Import abgeschlossen.":"Workbook-Export wurde erstellt.");
      else setError(event.data.error||"Workbook-Aktion fehlgeschlagen.");
    };
  ...
  const stepById=useMemo(()=>new Map<string,ProcessStep>((data?.processSteps||[]).map(s=>[s.id,s] as const)),[data]);
  const roleById=useMemo(()=>new Map<string,RR>((data?.responsibilityRoles||[]).map(r=>[r.id,r] as const)),[data]);
  const phaseForTask=(t:AdminTask)=>{
    const code=t.process_step_id?stepById.get(t.process_step_id)?.code||"":"";
    const raw=code||t.source_number||"";
    const n=Number(String(raw).match(/^[1-8]/)?.[0]||0);
    return n>=1&&n<=8?n:0;
  };
  const sortedMeasures=useMemo(()=>{
    ...
    rows.sort((a,b)=>{
      const ap=phaseForTask(a),bp=phaseForTask(b);
      if(ap!==bp)return ap-bp;
      if(measureSort==="due"){const av=a.due_date||"9999-12-31",bv=b.due_date||"9999-12-31";if(av!==bv)return av.localeCompare(bv);}
      if(measureSort==="role"){const ar=a.responsibility_role_id?roleById.get(a.responsibility_role_id)?.display_name||"":"";const br=b.responsibility_role_id?roleById.get(b.responsibility_role_id)?.display_name||"":"";const c=coll.compare(ar,br);if(c)return c;}
      if(measureSort==="process"){const ac=a.process_step_id?stepById.get(a.process_step_id)?.code||"":"";const bc=b.process_step_id?stepById.get(b.process_step_id)?.code||"":"";const c=coll.compare(ac,bc);if(c)return c;}
      return coll.compare(a.source_number||"",b.source_number||"")||coll.compare(a.title,b.title);
    });
    return rows;
  },[measures,measureSort,stepById,roleById]);
  const measureGroups=useMemo(()=>Array.from({length:8},(_,i)=>{const phase=i+1;return {phase,rows:sortedMeasures.filter(t=>phaseForTask(t)===phase)};}),[sortedMeasures,stepById]);
  const phaseNames:Record<number,string>={1:"Prüfungsauftrag & Mandatierung",2:"Projektplanung & Kick-off",3:"Abschlussvorbereitung",4:"Erstellung Einzelabschlüsse",5:"Konsolidierung & Gruppenabschluss",6:"Prüfungsdurchführung",7:"Feststellung & Gremienbeschluss",8:"Veröffentlichung"};
  const toggleMeasurePhase=(phase:number)=>setOpenMeasurePhases(prev=>{const next=new Set(prev);next.has(phase)?next.delete(phase):next.add(phase);return next;});
  ...
  function buildAdminSnapshot(){
    ...
    return {format:"LUMINA_ADMIN_SNAPSHOT_V1",exportedAt:new Date().toISOString(),users:data.users.map(u=>({email:u.email,firstName:u.firstName,lastName:u.lastName,displayName:u.displayName})),companies:data.companies,projects:data.projects,processSteps:data.processSteps,responsibilityRoles:data.responsibilityRoles,tasks:data.tasks,companyMembers:data.companyMembers.map(m=>({...m,email:emailById.get(m.user_id)||""})),projectMembers:data.projectMembers.map(m=>({...m,email:emailById.get(m.user_id)||""})),roleAssignments:data.roleAssignments.map(a=>({role_id:a.role_id,email:emailById.get(a.user_id)||""}))};
  }
  async function importAdminJson(file:File){
    try{const bundle=JSON.parse(await file.text());const result=await act({action:"import_admin_snapshot",bundle},"JSON-Datenstand wirklich importieren und bestehende Datensätze aktualisieren?");if(result){setTransferMessage(`JSON-Import abgeschlossen: ${result.projects||0} Projekte, ${result.tasks||0} Maßnahmen.`);if(Array.isArray(result.createdUsers)&&result.createdUsers.length)window.alert(`Neu angelegte Benutzer – Einmalpasswörter bitte sichern:

${result.createdUsers.map((u:{email:string;temporaryPassword:string})=>`${u.email}: ${u.temporaryPassword}`).join("\n")}`);}}catch(e:unknown){setError(e instanceof Error?e.message:"JSON-Datei konnte nicht importiert werden.");}
  }
  async function sendWorkbookSessionAndCommand(type:string,payload:Record<string,unknown>={}){
    const frame=workbookFrameRef.current;if(!frame?.contentWindow){setError("Workbook-Modul ist noch nicht bereit.");return;}
    const supabase=createClient();const {data:sessionData}=await supabase.auth.getSession();const session=sessionData.session;if(!session){setError("Keine aktive Sitzung.");return;}
    frame.contentWindow.postMessage({type:"lumina-session",accessToken:session.access_token,refreshToken:session.refresh_token},window.location.origin);
    window.setTimeout(()=>frame.contentWindow?.postMessage({type,...payload},window.location.origin),900);
  }
  async function importWorkbook(file:File){const buffer=await file.arrayBuffer();await sendWorkbookSessionAndCommand("lumina-admin-workbook-import",{name:file.name,buffer});setTransferMessage("Workbook-Import wurde an das bestehende LUMINA-Workbook-Modul übergeben.");}
  ...
  async function saveTask(){
    ...
    if(result){setTaskForm(emptyTaskForm);setNewTaskMode(false);}
  }

  function renderMeasureEditor(){return <div className="formPanel adminMeasureFormInline"><div className="adminMeasureEditorGrid"><label>Nr.<input value={taskForm.sourceNumber} onChange={e=>setTaskForm({...taskForm,sourceNumber:e.target.value})}/></label><label>Prozessschritt<select value={taskForm.processStepId} onChange={e=>setTaskForm({...taskForm,processStepId:e.target.value})}><option value="">Ohne Zuordnung</option>{activeSteps.map(s=><option key={s.id} value={s.id}>{s.code} · {s.name}</option>)}</select></label><label>Workflow-Rolle<select value={taskForm.responsibilityRoleId} onChange={e=>setTaskForm({...taskForm,responsibilityRoleId:e.target.value})}><option value="">Nicht zugeordnet</option>{activeRoles.map(r=><option key={r.id} value={r.id}>{r.display_name}</option>)}</select></label><label>Fälligkeit<input type="date" value={taskForm.dueDate} onChange={e=>setTaskForm({...taskForm,dueDate:e.target.value})}/></label></div><label>Aufgabe<textarea className="adminAutoTextarea" rows={3} value={taskForm.title} onChange={e=>setTaskForm({...taskForm,title:e.target.value})}/></label><label>Benötigte Unterlagen<textarea className="adminAutoTextarea" rows={4} value={taskForm.requiredDocuments} onChange={e=>setTaskForm({...taskForm,requiredDocuments:e.target.value})}/></label><div className="adminMeasureEditorGrid"><label>Bearbeitung<select value={taskForm.workStatus} onChange={e=>setTaskForm({...taskForm,workStatus:e.target.value})}><option value="open">Offen</option><option value="in_progress">In Bearbeitung</option><option value="submitted">Eingereicht</option><option value="completed">Abgeschlossen</option><option value="not_relevant">Nicht relevant</option></select></label><label>Review<select value={taskForm.reviewStatus} onChange={e=>setTaskForm({...taskForm,reviewStatus:e.target.value})}><option value="unreviewed">Ungeprüft</option><option value="question">Rückfrage</option><option value="changes_required">Nachbesserung</option><option value="accepted">Akzeptiert</option></select></label></div><div className="adminMeasureEditorActions"><button className="primaryButton" disabled={busy} onClick={saveTask}>{taskForm.id?"Maßnahme speichern":"Maßnahme anlegen"}</button>{taskForm.id&&<button className="dangerButton" disabled={busy} onClick={async()=>{const r=await act({action:"delete_task",taskId:taskForm.id},"Maßnahme endgültig löschen?");if(r)setTaskForm(emptyTaskForm);}}>Maßnahme löschen</button>}<button className="secondaryAdminButton" type="button" onClick={()=>{setTaskForm(emptyTaskForm);setNewTaskMode(false);}}>Abbrechen</button></div></div>}
  ...
  const title=tab==="users"?"Benutzer verwalten":tab==="companies"?"Gesellschaften":tab==="projects"?"Projekte":tab==="measures"?"Maßnahmen":tab==="transfer"?"Import / Export":"Rollen & Berechtigungen";
  ...
  return <main className={embedded?"adminEmbedded":"adminPage"}>
    ...
    {embedded&&<div className="adminEmbeddedTitle"><div><button type="button" className="adminBack" onClick={onBack}>← Administration</button><h1>{title}</h1><p>{tab==="measures"?`${activeProject?.name||"Aktives Projekt"} · ${measures.length} Maßnahmen`:tab==="transfer"?"Superadmin Datentransfer für LUMINA und das vorhandene Abschluss-Workbook.":"Zentrale Verwaltung innerhalb der LUMINA-Arbeitsoberfläche."}</p></div></div>}
    ...
    <div className="adminToolbar"><div className="adminTabs">
      ...
      {isSuperAdmin&&<button className={tab==="transfer"?"active":""} onClick={()=>setTab("transfer")}>Import / Export</button>}
    ...
    {tab==="measures"&&<section className="adminMeasuresAccordionWorkspace">
      <div className="adminPanel adminMeasuresMain">
        <div className="panelHead adminMeasureHead"><div><h2>Maßnahmen · {activeProject?.name||"Aktives Projekt"}</h2><p className="formHint">Nach Prozessphase 1–8 gruppiert. Beim Start sind alle Gruppen geschlossen.</p></div><div className="buttonRow compact"><span className="adminMeasureCount">{measures.length} Maßnahmen</span><button onClick={()=>{setTaskForm(emptyTaskForm);setNewTaskMode(true);}}>+ Maßnahme</button></div></div>
        <div className="adminMeasureSortbar"><span>Sortieren nach</span>{([['process','Prozessschritt'],['due','Fälligkeit'],['role','zuständige Rolle'],['number','Maßnahmen-Nr.']] as [MeasureSort,string][]).map(([key,label])=><button key={key} className={measureSort===key?'active':''} onClick={()=>setMeasureSort(key)}>{label}</button>)}</div>
        {!taskForm.id&&!newTaskMode&&<div className="adminMeasureNewHint">Neue Maßnahme über <b>+ Maßnahme</b> anlegen oder eine vorhandene Maßnahme im Akkordeon öffnen.</div>}
        <div className="adminMeasureAccordion">{measureGroups.map(group=>{const open=openMeasurePhases.has(group.phase);return <section className="adminMeasurePhase" key={group.phase}><button className="adminMeasurePhaseButton" onClick={()=>toggleMeasurePhase(group.phase)} aria-expanded={open}><span className="adminMeasureChevron">{open?'▾':'▸'}</span><b>{group.phase}. {phaseNames[group.phase]}</b><span>{group.rows.length} Maßnahmen</span></button>{open&&<div className="adminMeasurePhaseBody">{group.rows.length===0?<p className="empty">Keine Maßnahmen in diesem Prozessschritt.</p>:group.rows.map(t=>{const step=t.process_step_id?stepById.get(t.process_step_id):undefined;const role=t.responsibility_role_id?roleById.get(t.responsibility_role_id):undefined;const selected=taskForm.id===t.id;return <div className={`adminMeasureAccordionItem ${selected?'selected':''}`} key={t.id}><button className="adminMeasureSummary" onClick={()=>editTask(t)}><span className="adminMeasureNr">{t.source_number||'–'}</span><span className="adminMeasureTitle"><b>{t.title}</b><small>{step?.code||'–'} · {step?.name||'Ohne Prozessschritt'}</small></span><span className="adminMeasureRole">{role?.display_name||'Nicht zugeordnet'}</span><span className="adminMeasureDue">{fmt(t.due_date)}</span><span>Bearbeiten</span></button>{selected&&<div className="adminInlineMeasureEditor">{renderMeasureEditor()}</div>}</div>})}</div>}</section>})}</div>
      </div>
      {newTaskMode&&<div className="adminPanel adminNewMeasureEditor"><h2>Neue Maßnahme</h2>{renderMeasureEditor()}</div>}
    </section>}
    ...
    {tab==="transfer"&&isSuperAdmin&&<section className="adminTransferWorkspace">
      <div className="adminPanel"><h2>JSON Import / Export</h2><p className="formHint">Sichert die zentralen LUMINA Verwaltungs- und Workflowdaten. Auth-Passwörter und Dateibinärdaten sind nicht Bestandteil der JSON-Datei.</p><div className="adminTransferActions"><button className="primaryButton" onClick={()=>{const snapshot=buildAdminSnapshot();if(snapshot)downloadJson(`LUMINA_Gesamtdaten_${new Date().toISOString().slice(0,10)}.json`,snapshot);}}>Gesamtdaten als JSON exportieren</button><button className="secondaryAdminButton" onClick={()=>jsonImportRef.current?.click()}>JSON importieren</button><input ref={jsonImportRef} type="file" accept="application/json,.json" hidden onChange={async e=>{const file=e.target.files?.[0];e.target.value="";if(file)await importAdminJson(file);}}/></div></div>
      <div className="adminPanel"><h2>Excel-Workbook Import / Export</h2><p className="formHint">Verwendet ausdrücklich das bereits vorhandene LUMINA Abschluss-Workbook mit <b>00_Inhaltsverzeichnis</b>, Hyperlinks, Rücksprunglinks und <b>_LUMINA_META</b>. Es wird kein zweites Workbook erzeugt.</p><div className="adminTransferProject"><b>Aktives Projekt</b><span>{activeProject?.name||"Kein Projekt ausgewählt"}</span></div><div className="adminTransferActions"><button className="primaryButton" disabled={!activeProjectId} onClick={()=>sendWorkbookSessionAndCommand("lumina-admin-workbook-export")}>Bestehendes Gesamt-Workbook exportieren</button><button className="secondaryAdminButton" disabled={!activeProjectId} onClick={()=>workbookImportRef.current?.click()}>Gesamt-Workbook importieren</button><input ref={workbookImportRef} type="file" accept=".xlsx" hidden onChange={async e=>{const file=e.target.files?.[0];e.target.value="";if(file)await importWorkbook(file);}}/></div>{transferMessage&&<div className="formSuccess">{transferMessage}</div>}<iframe ref={workbookFrameRef} className="adminWorkbookBridge" src={`/legacy/lumina.html${activeProjectId?`?project=${encodeURIComponent(activeProjectId)}`:""}`} title="LUMINA Workbook Bridge" onLoad={()=>void sendWorkbookSessionAndCommand("lumina-admin-ping")}/></div>
    </section>}
